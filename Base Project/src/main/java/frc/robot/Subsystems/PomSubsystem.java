...
public abstract class PomSubsystem extends SubsystemBase {
     ...
     */
    private static final List<PomSubsystem> SUBSYSTEMS = new ArrayList<>();
    private static final Trigger DISABLE_TRIGGER = new Trigger(DriverStation::isDisabled);

    static {
        DISABLE_TRIGGER.onTrue((new InstantCommand(() -> forEach(PomSubsystem::stopMotor)).andThen(() -> forEach(subsystem -> subsystem.setIdleMode(false)))).ignoringDisable(true));
    ...
    public static void forEach(Consumer<PomSubsystem> toRun) {
